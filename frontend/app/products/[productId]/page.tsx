    ...
    return (
      <OwnerShell>
        <div className="product-admin-page p-8 max-w-xl mx-auto text-center space-y-4">
          ...
          <p className="text-sm text-gray-500">{error || "Unable to find the requested product."}</p>
          <Link
            href="/products"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-100 text-gray-900 text-sm"
          >
      ...
      <div className="product-admin-page p-6 max-w-5xl mx-auto space-y-8">
        {actionError && (
          <div className="p-3.5 rounded-xl border border-red-500/30 bg-red-500/10 text-red-200 text-sm flex items-center justify-between">
            ...
            <button type="button" onClick={() => setActionError(null)} className="text-red-300 hover:text-gray-900">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-sm text-gray-500">
            <Link href="/products" className="hover:text-gray-900 flex items-center gap-1 transition">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#4854A8] hover:bg-[#374191] text-white text-xs font-medium transition shadow-sm"
          >
        ...
        <div className="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-md bg-[#4854A8]/10 text-[#4854A8] border border-indigo-100">
                  {product.model_number}
                </span>
                <span className="text-xs text-gray-500">{product.category || "General"}</span>
              </div>
              <h1 className="text-2xl font-bold text-gray-900 mt-1.5">{product.name}</h1>
            </div>
            <span
          ...
          {product.support_disclaimer && (
            <div className="p-3 rounded-xl bg-gray-50 border border-gray-100 text-xs text-gray-500">
              <span className="text-gray-600 font-medium">Support Disclaimer: </span>
              {product.support_disclaimer}
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <FileText className="w-5 h-5 text-[#4854A8]" />
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-[#4854A8] hover:bg-[#374191] text-white text-xs font-medium transition shadow-sm"
            >
          ...
          {docs.length === 0 ? (
            <div className="p-8 rounded-xl border border-dashed border-gray-200 text-center space-y-2">
              ...
              <p className="text-sm text-gray-600 font-medium">No manuals assigned yet</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {docs.map((d) => (
                <div
                  key={d.document_id}
                  className="p-4 rounded-xl bg-white border border-gray-200 shadow-sm flex items-start justify-between gap-3"
                >
                  <div className="flex items-start gap-3 min-w-0">
                    <FileText className="w-5 h-5 text-[#4854A8] shrink-0 mt-0.5" />
        ...
        <div className="space-y-4 pt-4 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                <QrCode className="w-5 h-5 text-[#4854A8]" />
          ...
          {links.length === 0 ? (
            <div className="p-8 rounded-xl border border-dashed border-gray-200 text-center space-y-2">
              ...
              <p className="text-sm text-gray-600 font-medium">No QR links generated</p>
              <p className="text-xs text-gray-500 max-w-sm mx-auto">
            ...
            <div className="space-y-3">
              {links.map((link) => (
                <div
                  key={link.link_id}
                  className="p-4 rounded-xl bg-white border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-900">{link.label || link.name}</span>
                      <span
            ...
            <div className="product-admin-dialog bg-white border border-gray-200 shadow-sm rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                <h3 className="text-base font-semibold text-gray-900 flex items-center gap-2">
                  <Edit2 className="w-5 h-5 text-[#4854A8]" />
                  Edit Product SKU
                </h3>
                <button onClick={() => setEditModalOpen(false)} className="text-gray-500 hover:text-gray-900">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <form onSubmit={handleEditSave} className="space-y-4 text-sm">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#4854A8]/20 focus:border-[#4854A8]"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Model / SKU #</label>
                    <input
                      type="text"
                      required
                      value={editModel}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#4854A8]/20 focus:border-[#4854A8] resize-none"
                  />
                </div>

                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Support Disclaimer</label>
                  <input
                    type="text"
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-gray-200">
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl text-gray-600 text-xs"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={savingEdit}
                    className="px-4 py-1.5 rounded-xl bg-[#4854A8] hover:bg-[#374191] text-white text-xs font-medium transition"
                  >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm backdrop-blur-sm">
            <div className="product-admin-dialog bg-white border border-gray-200 shadow-sm rounded-2xl w-full max-w-sm max-h-[calc(100dvh-2rem)] overflow-y-auto p-5 shadow-2xl space-y-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                ...
                <button onClick={() => setDocModalOpen(false)} className="text-gray-500 hover:text-gray-900">
                  <X className="w-4 h-4" />
                </button>
              </div>
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#4854A8]/20 focus:border-[#4854A8]"
                    >
                      {allDocs.map((d) => (
                        <option key={d.document_id} value={d.document_id} className="bg-white text-gray-900">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm backdrop-blur-sm">
            <div className="product-admin-dialog bg-white border border-gray-200 shadow-sm rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                ...
                <button onClick={() => setQrModalOpen(false)} className="text-gray-500 hover:text-gray-900">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {!generatedLink ? (
                <form onSubmit={handleGenerateQr} className="space-y-4 text-sm">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 shadow-sm text-gray-900 placeholder:text-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-[#4854A8]/20 focus:border-[#4854A8]"
                    />
                  </div>
                ...
                <div className="space-y-3 text-center">
                  ...
                  <div className="p-3 rounded-xl bg-black/40 border border-gray-200 flex items-center justify-between gap-2 text-left">
                    <code className="text-xs text-[#4854A8] truncate select-all">
                      {window.location.origin}{generatedLink.url}
                    </code>
                    <button
                      onClick={() => copyToClipboard(`${window.location.origin}${generatedLink.url}`)}
                      className="p-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-900 shrink-0 transition"
                      title="Copy URL"
                    >
                  ...
                  <div className="flex flex-wrap justify-center gap-2 pt-1">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#4854A8] hover:bg-[#374191] text-white text-xs font-medium transition"
                    >
                    ...
                    <button
                      onClick={() => setQrModalOpen(false)}
                      className="px-4 py-2 rounded-xl bg-gray-100 text-gray-900 text-xs font-medium transition"
                    >
                      Done
                    </button>
                  </div>
